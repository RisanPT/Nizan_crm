...
Future<void> downloadLeadsReport({
  ...
  final bytes = await pdf.save();
  final tempDir = await getTemporaryDirectory();
  final file = File('${tempDir.path}/leads-report-${DateTime.now().year}-${DateTime.now().month.toString().padLeft(2, '0')}-${DateTime.now().day.toString().padLeft(2, '0')}.pdf');
  await file.writeAsBytes(bytes);
  await Share.shareXFiles(
    [XFile(file.path)],
    subject: 'Leads Report',
    text: 'Here is your leads report.',
    sharePositionOrigin: const Rect.fromLTWH(0, 0, 300, 300),
  );
}
