...
const _metricFields = <(String, String)>[
  ('followers', 'Followers'),
...
class _CompetitorsScreenState extends ConsumerState<CompetitorsScreen> {
      ...
      builder: (dctx) => StatefulBuilder(builder: (dctx, setLocal) {
        Future<void> save() async {
          ...
          try {
            ...
            if (dctx.mounted) Navigator.pop(dctx);
            messenger.showSnackBar(
                const SnackBar(content: Text('Weekly data saved')));
          } catch (e) {
            setLocal(() => saving = false);
            messenger.showSnackBar(SnackBar(content: Text('$e')));
          }
        ...
        return AlertDialog(
          title: Row(children: [
            Expanded(child: Text('Weekly data · ${c.name}')),
  ...
  String _metricValue(CompetitorSnapshot? s, String key) {
    ...
    switch (key) {
      case 'followers':
        return s.followers == 0 ? '' : s.followers.toStringAsFixed(0);
      case 'weeklyGrowthPct':
        return s.weeklyGrowthPct == 0 ? '' : '${s.weeklyGrowthPct}';
      case 'engagementRate':
        return s.engagementRate == 0 ? '' : '${s.engagementRate}';
      case 'adCampaigns':
        return s.adCampaigns == 0 ? '' : '${s.adCampaigns}';
      case 'offers':
        return s.offers == 0 ? '' : '${s.offers}';
      case 'newServicesCount':
        return s.newServicesCount == 0 ? '' : '${s.newServicesCount}';
      case 'postingFrequency':
        return s.postingFrequency == 0 ? '' : '${s.postingFrequency}';
      case 'seoScore':
        return s.seoScore == 0 ? '' : '${s.seoScore}';
      case 'reviews':
        return s.reviews == 0 ? '' : '${s.reviews}';
      case 'collaborations':
        return s.collaborations == 0 ? '' : '${s.collaborations}';
      default:
        return '';
    }
  }
