...
class _ITProjectsScreenState extends ConsumerState<ITProjectsScreen> {
                    ...
                    if (projects.isEmpty)
                      ...
                      LayoutBuilder(builder: (ctx, c) {
                        ...
                        return Wrap(
                          spacing: gap,
                          runSpacing: gap,
                          children: [for (final p in filtered) SizedBox(width: cardW, child: _projectCard(context, ref, crm, p))],
                        );
                      }),
                  ],
                ),
  ...
  Widget _emptyState(CrmTheme crm, BuildContext context, WidgetRef ref) => Padding(
        padding: const EdgeInsets.only(top: 60),
        child: Center(
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Icon(Icons.folder_open_outlined, size: 52, color: crm.textSecondary),
            const SizedBox(height: 12),
            Text('No projects yet', style: TextStyle(color: crm.textPrimary, fontWeight: FontWeight.w800, fontSize: 15)),
            const SizedBox(height: 4),
            Text('Create a project, then plan it with the WBS & Gantt tabs.', style: TextStyle(color: crm.textSecondary, fontSize: 12.5)),
